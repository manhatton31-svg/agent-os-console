...
export function TeammatesView() {
  const { teammates, spawn, retire, handoff } = useOs();
  ...
  return (
    <div className="grid gap-6">
      <header>
        <p className="text-[11px] uppercase tracking-[0.16em] text-faint">Grok Bot pattern</p>
        <h1 className="mt-1 font-display text-3xl tracking-tight">Teammates with their own VMs</h1>
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted">
          Each bot keeps isolated compute, a model/effort pair, and a private
          context budget. They message over the bus. You stay the escalation
          point — not the orchestrator of every step.
        </p>
      </header>
